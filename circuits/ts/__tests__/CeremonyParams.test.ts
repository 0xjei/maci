...
import { generateRandomIndex, getSignal, circomkit } from "./utils/utils";
...
describe("Ceremony param tests", () => {
  ...
  describe("ProcessMessage circuit", function test() {
    ...
    before(async () => {
      circuit = await circomkit.WitnessTester("processMessages", {
        file: "processMessages",
        template: "ProcessMessages",
        params: [6, 8, 2, 3],
      });

      hasherCircuit = await circomkit.WitnessTester("processMessageInputHasher", {
        file: "processMessages",
    ...
    describe("TallyVotes circuit", function test() {
      ...
      before(async () => {
        testCircuit = await circomkit.WitnessTester("tallyVotes", {
          file: "tallyVotes",
          template: "TallyVotes",
          params: [6, 2, 3],
        });
