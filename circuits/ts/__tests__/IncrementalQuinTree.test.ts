...
describe("IncrementalQuinTree circuit", function test() {
  ...
  before(async () => {
    circuitLeafExists = await circomkit.WitnessTester("quinLeafExists", {
      file: "./trees/incrementalQuinTree",
      template: "QuinLeafExists",
      params: [3],
    });

    circuitGeneratePathIndices = await circomkit.WitnessTester("quinGeneratePathIndices", {
      file: "./trees/incrementalQuinTree",
      template: "QuinGeneratePathIndices",
      params: [4],
    });

    circuitQuinSelector = await circomkit.WitnessTester("quinSelector", {
      file: "./trees/incrementalQuinTree",
      template: "QuinSelector",
      params: [5],
    });

    splicerCircuit = await circomkit.WitnessTester("splicer", {
      file: "./trees/incrementalQuinTree",
      template: "Splicer",
      params: [4],
    });
