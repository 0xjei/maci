...
export const generateZkeys = async (outputPath?: string): Promise<void> => {
  ...
  for (let i = 0; i < circuitsConfigs.length; i += 1) {
    ...
    // rename to zkey as circomkit changes to pkey and move to the output folder
    if (outPath) {
      fs.renameSync(proverKeyPath, path.resolve(outPath, `${circuit.name}.0.zkey`));
    } else {
      fs.renameSync(proverKeyPath, `${proverKeyPath}.0.zkey`);
    }
  }
